...
Ricardo::ConfigVar<std::vector<int>>::ptr g_int_vec_value_config =
    Ricardo::Config::Lookup("system.int_vec", std::vector<int>{1, 2},
                            "system int vec");

Ricardo::ConfigVar<std::list<int>>::ptr g_int_list_value_config =
    Ricardo::Config::Lookup("system.int_list", std::list<int>{1, 2},
                            "system int list");

Ricardo::ConfigVar<std::set<int>>::ptr g_int_set_value_config =
    Ricardo::Config::Lookup("system.int_set", std::set<int>{1, 2},
                            "system int set");

Ricardo::ConfigVar<std::unordered_set<int>>::ptr g_int_uset_value_config =
    Ricardo::Config::Lookup("system.int_uset", std::unordered_set<int>{1, 2},
                            "system int uset");

Ricardo::ConfigVar<std::map<std::string, int>>::ptr g_str_int_map_value_config =
    Ricardo::Config::Lookup("system.str_int_map",
                            std::map<std::string, int>{{"k", 2}},
                            "system str int map");

Ricardo::ConfigVar<std::unordered_map<std::string, int>>::ptr
    g_str_int_umap_value_config = Ricardo::Config::Lookup(
...
void print_yaml(const YAML::Node& node, int level) {
  if (node.IsScalar()) {
    ...
    ICEY_LOG_INFO(ICEY_LOG_ROOT()) << std::string(level * 4, ' ') << "NULL - "
                                   << node.Type() << " - " << level;
  } else if (node.IsMap()) {
...
void test_yaml() {
  YAML::Node node =
      YAML::LoadFile("/home/Ricardo/ICEY/Ricardo/tests/yaml/test.yml");
  print_yaml(node, 0);
...
void test_config() {
  ICEY_LOG_INFO(ICEY_LOG_ROOT())
      << "brfore: " << g_int_value_config->getValue();
  ICEY_LOG_INFO(ICEY_LOG_ROOT())
      << "brfore: " << g_float_value_config->toString();
...
#define XX_M(g_var, name, prefix)                                            \
  ...
  YAML::Node root = YAML::LoadFile(
      "/home/Ricardo/Object/NetWork/Socket/SocketForCpp/tests/yaml/test.yml");
...
class Person {
 public:
  ...
  std::string toString() const {
    std::stringstream ss;
    ss << "[Person name = " << m_name << " arg = " << m_age
       << " sex = " << m_sex << "]";
    return ss.str();
  }
...
Ricardo::ConfigVar<std::map<std::string, Person>>::ptr g_person_map =
    Ricardo::Config::Lookup("class.map", std::map<std::string, Person>(),
                            "system person");

Ricardo::ConfigVar<std::map<std::string, std::vector<Person>>>::ptr
    g_person_vec_map = Ricardo::Config::Lookup(
...
#define XX_PM(g_var, prefix)                                             \
  ...
  ICEY_LOG_INFO(ICEY_LOG_ROOT()) << "after: " << g_person->getValue().toString()
                                 << " - " << g_person->toString();
