...
static Ricardo::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    Ricardo::Config::Lookup("http.request.buffer_size", 4 * 1024ul,
                            "http request buffer size");

static Ricardo::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
...
void on_request_method(void* data, const char* at, size_t length) {
  ...
  if (m == HttpMethod::INVALID_METHOD) {
    ICEY_LOG_WARN(g_logger)
        << "invald http request method" << std::string(at, length);
...
HttpRequestParser::HttpRequestParser() : m_error(0) {
  m_data.reset(new Ricardo::http::HttpRequest);
...
HttpResponseParser::HttpResponseParser() : m_error(0) {
  m_data.reset(new Ricardo::http::HttpResponse);
