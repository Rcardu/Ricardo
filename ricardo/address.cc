...
bool Address::GetInterfaceAddresses(
  ...
  if(getifaddrs(&results) != 0){
    ICEY_LOG_ERROR(g_logger)<<"Address::GetInterfaceAddresses getifaddrs "<<
  ...
  try{
    ...
    ICEY_LOG_ERROR(g_logger)<<"Address::GetInterfaceAddresses excepton";
    freeifaddrs(results);
    return false;
  }
  freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port){
  ...
  if(error){
    ICEY_LOG_ERROR(g_logger) << "IPAddress::Create(" << address << ", " << port
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
  ...
  if(result <= 0){
    ICEY_LOG_ERROR(g_logger) << "IPv6Address::Create(" << address << ", "
...
IPv6Address::IPv6Address(const uint8_t address[16], uint16_t port) {
  ...
  int rt = memcmp(&m_addr.sin6_addr.s6_addr, address, 16);
}
...
IPAddress::ptr IPv6Address::subnetMask(uint32_t prefix_len) {
  ...
  memset(&subnet, 0, sizeof(subnet));
  subnet.sin6_addr.s6_addr[prefix_len / 8] = ~CreateMask<uint8_t>(prefix_len);

  for (int i = 0; i < prefix_len / 8 + 1; i++) {
...
std::ostream& UnknowAddress::insert(std::ostream& os) const {
  os << "[UnixAddress family= " << m_addr.sa_family << "]";
  return os;
}
